...
public class FinancialService {
	...
	public FinancialResponse createFinancial(
		...
		Financial financial = Financial.builder()
				...
				.revenueYear1(request.getRevenueYear1())
				.revenueYear2(request.getRevenueYear2())
				.revenueYear3(request.getRevenueYear3())
				.ebitdaYear1(request.getEbitdaYear1())
				.ebitdaYear2(request.getEbitdaYear2())
				.ebitdaYear3(request.getEbitdaYear3())
				.assets(request.getAssets())
				.liabilities(request.getLiabilities())
	...
	public List<FinancialResponse> getAllFinancials() {
		...
		List<Financial> financials =
				financialRepository.findAll();

		List<FinancialResponse> responses =
				new ArrayList<>();

		for (Financial financial : financials) {
			responses.add(
					mapToResponse(financial));
		}
	...
	public FinancialResponse updateFinancial(
		...
		financial.setRevenueYear1(
				request.getRevenueYear1());

		financial.setRevenueYear2(
				request.getRevenueYear2());

		financial.setRevenueYear3(
				request.getRevenueYear3());

		financial.setEbitdaYear1(
				request.getEbitdaYear1());

		financial.setEbitdaYear2(
				request.getEbitdaYear2());

		financial.setEbitdaYear3(
				request.getEbitdaYear3());

		financial.setAssets(
				request.getAssets());

		financial.setLiabilities(
				request.getLiabilities());

		financial.setUpdatedAt(
				LocalDateTime.now());
	...
	private FinancialResponse mapToResponse(
			Financial financial) {

		return FinancialResponse.builder()
				.id(financial.getId())
				.companyId(financial.getCompanyId())
				.revenueYear1(financial.getRevenueYear1())
				.revenueYear2(financial.getRevenueYear2())
				.revenueYear3(financial.getRevenueYear3())
				.ebitdaYear1(financial.getEbitdaYear1())
				.ebitdaYear2(financial.getEbitdaYear2())
				.ebitdaYear3(financial.getEbitdaYear3())
